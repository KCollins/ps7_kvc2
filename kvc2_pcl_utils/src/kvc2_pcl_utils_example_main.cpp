//kvc2_pcl_utils_example_main.cpp
// show use of kvc2PclUtils library, using public functions:
...
int main(int argc, char** argv) {
    ros::init(argc, argv, "kvc2_pcl_utils_example_main"); //node name
    ros::NodeHandle nh;
    ...
    while (ros::ok()) {
        if (kvc2_pcl_utils.got_selected_points()) {
            ...
            //kvc2_pcl_utils.get_transformed_selected_points(display_cloud);
            //alternative: compute and get offset points from selected, transformed points
            kvc2_pcl_utils.example_pcl_operation(); // offset the transformed, selected points and put result in gen-purpose object
