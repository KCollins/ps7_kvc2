...
int main(int argc, char** argv) {
    ros::init(argc, argv, "kvc2_pcl_utils_color_test"); //node name
    ros::NodeHandle nh;
    kvc2PclUtils kvc2_pcl_utils(&nh);
    while (!kvc2_pcl_utils.got_kinect_cloud()) {
    ...
    A_sensor_wrt_torso = kvc2_pcl_utils.transformTFToEigen(tf_sensor_frame_to_torso_frame);
    //transform the kinect data to the torso frame;
    // we don't need to have it returned; kvc2_pcl_utils can own it as a member var
    kvc2_pcl_utils.transform_kinect_cloud(A_sensor_wrt_torso);
    //save this transformed data to disk:
    //kvc2_pcl_utils.save_transformed_kinect_snapshot();
    ...
    while (ros::ok()) {
        if (kvc2_pcl_utils.got_selected_points()) {
            ROS_INFO("transforming selected points");
            kvc2_pcl_utils.transform_selected_points_cloud(A_sensor_wrt_torso);

            //fit a plane to these selected points:
            ROS_INFO("fitting a plane to the transformed selected points:");
            kvc2_pcl_utils.fit_xformed_selected_pts_to_plane(plane_normal, plane_dist);
            ROS_INFO_STREAM(" normal: " << plane_normal.transpose() << "; dist = " << plane_dist);
            centroid = kvc2_pcl_utils.get_centroid();
            ...
            kvc2_pcl_utils.copy_indexed_pts_to_output_cloud(selected_indices,display_color_cloud);            
            ...
            avg_color = kvc2_pcl_utils.find_avg_color_selected_pts(selected_indices);
            ...
            kvc2_pcl_utils.filter_cloud_z(plane_dist, z_eps, radius_large,centroid,selected_indices);
            
            ROS_INFO("extracting points colored similar to patch average: ");
            cout<<"enter color threshold (0 to 1): ";
            cin>>color_match_thresh;

            kvc2_pcl_utils.find_indices_color_match(selected_indices,
